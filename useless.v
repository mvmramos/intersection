...
(* --------------------------------------------------------------------- *)
(* SIMPLIFICATION - USELESS SYMBOLS - ALTERNATIVE DEFINITIONS            *)
(* --------------------------------------------------------------------- *)

Inductive useful2 (g: cfg non_terminal terminal): (non_terminal + terminal) -> Prop:=
| use_1: forall t: terminal,
         useful2 g (inr t)
| use_2: forall left: non_terminal,
         forall right: sf,
         rules g left right ->
...
Fixpoint useful3 (g: cfg non_terminal terminal) (s: non_terminal + terminal): Prop:=
match s with
| inl n => False
| inr t => True
end.
