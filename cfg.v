...
Lemma derives6_terminal_left:
forall g: cfg _ _,
forall n: nat,
forall t: terminal,
forall s: sf,
forall w: list terminal,
derives6 g (Datatypes.S n) ([inr t]++s) (map (terminal_lift non_terminal (terminal:=terminal)) w) ->
exists w': list terminal,
(w=[t]++w') /\ 
derives6 g (Datatypes.S n) s (map (terminal_lift non_terminal (terminal:=terminal)) w').
Proof.
intros g n t s w H1.
assert (H2: exists m: nat, 
            derives6 g m ([inr t] ++ s)
            (map (terminal_lift non_terminal (terminal:=terminal)) w)).
  {
  exists (Datatypes.S n).
  exact H1.
  }
apply <- derives_equiv_derives6 in H2.
apply derives_t_symbol_left in H2.
...
- simpl in H2.
  ...
  + rewrite <- H0 in H1.
    clear H0 H2 H3 s2' t.
    simpl map in H1.
    change (terminal_lift non_terminal t0:: map (terminal_lift non_terminal (terminal:=terminal)) w) with
           ([inr t0]++map (terminal_lift non_terminal (terminal:=terminal)) w) in H1.
    apply derives6_t_t in H1.
    exact H1.
Qed.

Lemma derives_t_symbol_right:
forall g: cfg _ _,
forall s1 s2: sf,
forall t: terminal,
derives g (s1++[inr t]) s2 ->
exists s2': sf, 
s2 = s2'++[inr t].
Proof.
intros g s1 s2 t H1.
replace (s1++[inr t]) with (s1++[inr t]) in H1.
- apply derives_split in H1.
  destruct H1 as [s1' [s2' [H2 [H3 H4]]]].
  exists s1'.
  apply derives_t_left_v2 in H4.
  rewrite H4 in H2.
  exact H2.
- reflexivity.
Qed.

Lemma derives6_t_t_right:
forall g: cfg _ _,
forall n: nat,
forall t: terminal,
forall s1 s2: sf,
derives6 g n (s1++[inr t]) (s2++[inr t]) -> derives6 g n s1 s2.
Proof.
intros g n t s1 s2 H.
apply derives6_split in H.
destruct H as [s1' [s2' [n1 [n2 [H1 [H2 [H3 H4]]]]]]].
assert (H5:=H4).
apply derives6_t_left in H4.
rewrite H4 in H1.
apply app_inj_tail in H1.
destruct H1 as [H1 _].
rewrite <- H1 in H3.
apply derives6_t_left_n in H5.
rewrite H2, H5.
rewrite plus_0_r. 
exact H3.
Qed.

Lemma derives6_terminal_right:
forall g: cfg _ _,
forall n: nat,
forall t: terminal,
forall s: sf,
forall w: list terminal,
derives6 g (Datatypes.S n) (s++[inr t]) (map (terminal_lift non_terminal (terminal:=terminal)) w) ->
exists w': list terminal,
(w=w'++[t]) /\ 
derives6 g (Datatypes.S n) s (map (terminal_lift non_terminal (terminal:=terminal)) w').
Proof.
...
apply <- derives_equiv_derives6 in H2.
apply derives_t_symbol_right in H2.
destruct H2 as [s2' H2].
assert (H3: w<>[]).
  {
  destruct w. 
  - simpl in H2.
    apply not_empty_eq_app_single in H2. 
    contradiction. 
  -intros H.
   inversion H.
  }
apply exists_last in H3.
destruct H3 as [l' [a H3]].
rewrite H3 in H2.
rewrite map_app in H2.
simpl in H2.
change ([terminal_lift non_terminal a]) with ([@inr non_terminal terminal a]) in H2.
...
- rewrite H3 in H1.
  ...
  simpl in H1.
  change ([terminal_lift non_terminal t]) with ([@inr non_terminal terminal t]) in H1.
  apply derives6_t_t_right in H1.
  exact H1.
Qed.

Lemma derives6_terminal_left_right:
forall g: cfg _ _,
forall n: nat,
forall t1 t2: terminal,
forall s: sf,
forall w: list terminal,
derives6 g (Datatypes.S n) ([inr t1]++s++[inr t2]) (map (terminal_lift non_terminal (terminal:=terminal)) w) ->
exists w': list terminal,
(w=[t1]++w'++[t2]) /\ 
derives6 g (Datatypes.S n) s (map (terminal_lift non_terminal (terminal:=terminal)) w').
Proof.
intros g n t1 t2 s w H1.
apply derives6_terminal_left in H1.
destruct H1 as [w' [H1 H2]].
rewrite H1.
clear H1.
apply derives6_terminal_right in H2.
destruct H2 as [w'0 [H2 H3]].
rewrite H2.
exists w'0.
split.
- reflexivity.
