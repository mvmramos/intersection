...
(* --------------------------------------------------------------------- *)
(* SIMPLIFICATION - INACCESSIBLE SYMBOLS - ALTERNATIVE DEFINITIONS       *)
(* --------------------------------------------------------------------- *)

Inductive accessible2 (g: cfg _ terminal): (non_terminal + terminal) -> Prop:=
| acc_1: accessible2 g (inl (start_symbol g))
| acc_2: forall left: non_terminal,
         forall right: sf,
         rules g left right ->
         accessible2 g (inl left) ->
         forall s: non_terminal+terminal,
         In s right ->
         accessible2 g s.

Lemma accessible_equiv_accessible2:
forall s: non_terminal+terminal,
forall g: cfg _ _,
accessible g s <-> accessible2 g s.
Proof.
intros s g.
split.
- intros H1.
  unfold accessible in H1.
  destruct H1 as [s1 [s2 H2]].
  remember [inl (start_symbol g)] as w1.
  remember (s1 ++ s :: s2) as w2.
  induction H2.
...
- intros H1.
  ...
  + exists [], [].
    simpl.
    constructor.
  + destruct IHaccessible2 as [s1 [s2 H2]].
    apply derives_step with (right:=right) in H2.
    * apply in_split in H0.
      ...
      {
      replace ((s1 ++ l1) ++ s :: l2 ++ s2) with (s1 ++ l1 ++ (s :: l2) ++ s2).
      - exact H2.
      - rewrite <- app_assoc.
        reflexivity.
      }
