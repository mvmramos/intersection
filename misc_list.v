...
- apply exists_length_gt_2 in H2.
  + exact H2.
  + exact def.
...
Lemma length_split_3:
forall A: Type,
forall i n: nat,
forall w x y z: list A,
length w = 3*n ->
w=x++y++z ->
length x = i ->
length y = i ->
...
Proof.
destruct i.
- intros n w x y z H1 H2 H3 H4 H5.
  apply length_zero in H3.
  apply length_zero in H4.
  apply length_zero in H5.
...
Lemma length_split_4:
forall A: Type,
forall i n: nat,
forall w v x y z: list A,
length w = 4*n ->
w=v++x++y++z ->
length v = i ->
length x = i ->
...
Proof.
destruct i.
- intros n w v x y z H1 H2 H3 H4 H5 H6.
  apply length_zero in H3.
  apply length_zero in H4.
  apply length_zero in H5.
  apply length_zero in H6.
...
forall w: list A,
length w > 0 -> 
(exists w': list A, exists a: A, w = [a] ++ w') /\
(exists w': list A, exists a: A, w = w' ++ [a]).
Proof.
intros A w H1.
split.
- destruct w.
  ...
  + exists w, a.
    reflexivity.
- apply length_not_zero in H1.
  apply not_eq_sym in H1.
  apply exists_last in H1.
  destruct H1 as [l' [a H1]].
  exists l', a.
  exact H1.
Qed.
...
Definition sublist {A: Type} (w1 w2: list A): Prop:=
exists p s: list A,
w1 = p ++ w2 ++ s.
...
induction w.
- exact H1.
- destruct IHw as [p [s IHw]].
  change ((a :: w) ++ w1) with (a :: (w ++ w1)).
  rewrite IHw.
  exists (a :: p),s.
  reflexivity.
Qed.
...
- intros w1 w2 H1. 
  ...
  exact H1.
- intros w1 w2 H1. 
  specialize (IHw (w1 ++ [a]) w2).
  apply sublist_last with (t:=a) in H1.
  specialize (IHw H1).
  change (w1 ++ a :: w) with (w1 ++ [a] ++ w).
  repeat rewrite <-app_assoc in IHw. 
  exact IHw.
Qed.
